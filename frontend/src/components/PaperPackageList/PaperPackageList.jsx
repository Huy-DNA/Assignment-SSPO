...
import {
  DELETE_PACKAGES_URL, GET_PACKAGES_URL, UPDATE_PACKAGES_URL, UPLOAD_PACKAGES_URL,
} from '../../constants/url';
...
export default function PaperPackageList() {
  ...
  const setEditMode = (id) => {
    setPackageList((pkgs) => pkgs.map(
      (pkg) => (pkg.id === id ? { ...pkg, inEditMode: true } : pkg),
    ));
  };

  const handleAddPackage = () => {
    setNextFakeId((fakeId) => fakeId + 1);
    setNewPackageList((pkgs) => [...pkgs, {
      fakeId: nextFakeId, name: '', price: 0, paperNo: 0, description: '', thumbnailUrl: '',
    }]);
  };
  ...
  const handleSave = async (id) => {
    ...
    try {
      const newPkg = await axios.post(
        UPDATE_PACKAGES_URL,
        { ...packageList[pkgIndex], createdAt: undefined, inEditMode: undefined },
      ).then(({ data }) => extractAPIResponse(data));

      setPackageList((pkgs) => {
        const index = packageList.findIndex(({ id: _id }) => _id === id);
        if (index === -1) {
          return pkgs;
        }
        ...
        return [
          ...pkgs.slice(0, index),
          { ...newPkg, inEditMode: false }, ...pkgs.slice(index + 1),
        ];
      });
    } catch (e) {
  ...
  const handleUpload = async (id) => {
    ...
    notify(NotificationStatus.WAITING);
    try {
      const newPkg = await axios.post(UPLOAD_PACKAGES_URL, {
        ...newPackageList[pkgIndex], inEditMode: undefined, createdAt: undefined, fakeId: undefined,
      })
        .then(({ data }) => extractAPIResponse(data));

      setNewPackageList((pkgs) => {
        const index = pkgs.findIndex(({ fakeId }) => fakeId === id);
        return [...pkgs.slice(0, index), ...pkgs.slice(index + 1)];
      });
  ...
  const handleNewPackagePaperNoChange = (id, newPaperNo) => {
    setNewPackageList((pkgs) => {
      ...
      const pkg = pkgs[pkgIndex];
      return [
        ...pkgs.slice(0, pkgIndex),
        { ...pkg, paperNo: newPaperNo },
        ...pkgs.slice(pkgIndex + 1),
      ];
    });
  };
            ...
            !packageList
              ? (
                ...
                <>
                  {
                    packageList.map(({
                      id, name, thumbnailUrl, description, price, paperNo, inEditMode,
                    }) => (
                      <Fragment key={id}>
                              ...
                              ? (
                                <>
                                  ...
                                  <p className="text-blue-800 font-bold text-lg mb-2">
                                    ₫
                                    <input type="number" className="bg-gray-100 border-gray-500 border-2 active:border-gray-500 rounded-lg p-1" value={price} onChange={(e) => handlePackagePriceChange(id, e.target.value)} />
                                  </p>
                                  <p className="font-light mb-2">
                                    Gồm
                                    <input type="number" min={0} className="p-1 bg-gray-100 font-bold border-gray-500 border-2 active:border-gray-500 rounded-lg" value={paperNo} onChange={(e) => handlePackagePaperNoChange(id, e.target.value)} />
                                    {' '}
                                    tờ giấy
                                  </p>
                                </>
                              ...
                              : (
                                <>
                                  ...
                                  <p className="font-light">
                                    Gồm
                                    <span className="font-bold">{ paperNo }</span>
                                    {' '}
                                    tờ giấy
                                  </p>
                                </>
                  ...
                  {
                    newPackageList.map(({
                      fakeId, name, thumbnailUrl, description, price, paperNo,
                    }) => (
                      <Fragment key={fakeId}>
                        ...
                        <div>
                          ...
                          <p className="text-blue-800 font-bold text-lg mb-2">
                            ₫
                            <input type="number" className="bg-gray-100 border-gray-500 border-2 active:border-gray-500 rounded-lg p-1" value={price} onChange={(e) => handleNewPackagePriceChange(fakeId, e.target.value)} />
                          </p>
                          <p className="font-light mb-2">
                            Gồm
                            <input type="number" min={0} className="p-1 bg-gray-100 font-bold border-gray-500 border-2 active:border-gray-500 rounded-lg" value={paperNo} onChange={(e) => handleNewPackagePaperNoChange(fakeId, e.target.value)} />
                            {' '}
                            tờ giấy
                          </p>
                        </div>
